    ...
    async run(context, { fetchedFields, employeeId = '100' }) {
      ...
      try {
        ...
        if (fetchedFields && fetchedFields.length && employeeId !== undefined) {
          ...
          // remove artifical fields from being requested from backend:
          const fetchedFieldsTrimmed = fetchedFields.filter(i => !i.name.endsWith("Training"));
          ...
          $page.variables.employee.ethicalTraining = "Yes";
          $page.variables.employee.securityTraining = "Yes";
        }
      } finally {
